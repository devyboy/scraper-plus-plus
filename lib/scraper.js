...
async function sendToGoogleSheets(listings, sheetUrl) {
  try {
    ...
    if (CONFIG.appendMode && hasStructure) {
      ...
      console.log("   Updated spreadsheet with new data");
    }
...
async function runScraper(redfinUrl, sheetUrl) {
  try {
    ...
    console.log(
      `   Found ${listings.length} listings, sending to Google Sheets...`
    );
    const spreadsheetUrl = await sendToGoogleSheets(listings, sheetUrl);
