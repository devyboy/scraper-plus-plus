...
class GoogleSheetsManager {
  ...
  async authenticate() {
    try {
      ...
      if (serviceAccountEmail && privateKey) {
        ...
        throw new Error(
          "No Google credentials found. Please set GOOGLE_API_KEY or GOOGLE_SERVICE_ACCOUNT_EMAIL and GOOGLE_PRIVATE_KEY in your .env file"
        );
      }
  ...
  buildFormattingRequests(rowCount, headersLength) {
    return [
      ...
      // Data rows formatting (normal text, left-aligned)
      {
        repeatCell: {
          range: {
            sheetId: 0,
  ...
  async appendToSpreadsheet(spreadsheetId, data, sheetName = "Sheet1") {
    try {
      ...
      const currentRows = response.data.values
        ...
        : 0;
      const hasHeaders = currentRows > 0;
      ...
      const existingIds = await this.getExistingListingIds(
        spreadsheetId,
        sheetName
      );
      const newData = data.filter(
        (listing) => listing.listingId && !existingIds.has(listing.listingId)
      );
      if (newData.length === 0) {
      ...
      // Update formatting for all rows including headers
      const totalRows = hasHeaders
        ? currentRows + rows.length
        : rows.length + 1;
      const requests = this.buildFormattingRequests(totalRows, headers.length);
      await this.sheets.spreadsheets.batchUpdate({
        spreadsheetId,
        resource: { requests },
      });
